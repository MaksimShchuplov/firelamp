...
// One-shot: mDNS and the version-check task must come up on whichever path
// first establishes the link — autoConnect() at boot, or serviceNetwork()'s
// reconnect after the AP was down during boot. Both call sites are Core 1, so
// the latch needs no synchronisation. Not driven from the WiFi event handler:
// that runs on the arduino_events task with a small stack, and MDNS.begin()
// allocates and spawns a task.
static void startNetServices() {
    ...
    else                       LOG_WARN("mDNS start failed — firelamp.local will not resolve");
    startAutoUpdateTask();
}
...
void startNetwork() {
    ...
    WiFi.onEvent([](WiFiEvent_t event, WiFiEventInfo_t info) {
        // Only fast-path the FIRST drop after a successful association. The
        // driver posts this same event for the failures of our own retries, so
        // zeroing unconditionally makes the WIFI_RETRY_MS guard always true and
        // serviceNetwork() re-fires WiFi.reconnect() every ~10 ms loop pass.
        // Touched only from the arduino_events task, so a plain static is correct.
        static bool wasConnected = false;
        if (event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED) {
            if (wasConnected) { wasConnected = false; wifiRetryAt = 0; }
...
void serviceNetwork() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        startNetServices();                       // no-op after the first call
    } else if (millis() - wifiRetryAt > WIFI_RETRY_MS) {
        wifiRetryAt = millis();
        WiFi.reconnect();
    }
}
